...
export function useWaterSparklesControls() {
  return useControls(
    "Water Sparkles",
    {
      ...
      spread:       { value: 120,   min: 5,   max: 120, step: 1,    label: "Spread Radius" },
      heightOffset: { value: 0.00, min: 0,   max: 2,   step: 0.01, label: "Height Offset" },
      Size: folder({
        minSize: { value: 1, min: 1, max: 150, step: 1, label: "Min Size" },
        maxSize: { value: 3, min: 1, max: 300, step: 1, label: "Max Size" },
      }, { collapsed: false }),
      Lifetime: folder({
      ...
      Appearance: folder({
        ...
        armSharpness: { value: 9.5, min: 1,   max: 40,  step: 0.5,  label: "Arm Sharpness" },
        armFalloff:   { value: 4.4,  min: 0.1, max: 8,   step: 0.1,  label: "Arm Falloff" },
